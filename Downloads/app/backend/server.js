...
// Generate & store SHA-256 hashed OTP; send via Twilio SMS in production
  ...
  try {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `Detect the language of this text. Respond in JSON only, no other text: {"language": "code", "name": "full name", "confidence": 0.95}\n\nText: "${text}"`,
        }],
      }),
    });
    ...
    if (!match) return res.status(500).json({ error: 'Detection failed' });
...
2. Translate to ${targetLanguage || 'English'}

Respond in JSON only, no other text:
{"detectedLanguage":"code","detectedLanguageName":"name","targetLanguage":"${targetLanguage || 'English'}","original":"${text.replace(/"/g, '\\"')}","translated":"translation here","confidence":0.95}`,
        }],
      }),
    });
    ...
    const data = await response.json();
    const match = data.content?.[0]?.text?.match(/\{[\s\S]*\}/);
    if (!match) return res.status(500).json({ error: 'Translation failed' });
...
// POST /api/transcribe-voice — placeholder, ready for Whisper integration
app.post('/api/transcribe-voice', verifyToken, async (req, res) => {
  try {
    ...
    // TODO: integrate OpenAI Whisper, Google Speech-to-Text, or AWS Transcribe
    res.json({
      transcription: 'Voice transcription placeholder — integrate with Whisper API',
      translated: targetLanguage ? 'Translation placeholder' : null,
